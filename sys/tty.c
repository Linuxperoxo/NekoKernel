...
static __u8 tty_write(offset_t __offset__, void *__buffer__) {
  ...
  default:
    ...
    vfs_write("/dev/video",
              (__tty->__win.__row * TTY_WIDTH * 2) + (__tty->__win.__col * 2),
              __buffer__);
    vfs_write("/dev/video",
              (__tty->__win.__row * TTY_WIDTH * 2) +
                  (__tty->__win.__col++ * 2) + 1,
              &__tty->__win.__color);
    break;
  }
...
void tty_init() {
  ...
  __tty->__win.__col = 0x00;
  __tty->__win.__color =
      (((TTY_DF_BC_COLOR << 4) & 0x70) | (TTY_DF_CH_COLOR & 0x0F));

  vfs_mkbcfile("/dev/tty", ROOT_UID, ROOT_GID, READ_O | WRITE_O, &tty_read,
               &tty_write);
}

void tty_switch(tty_t *__tty__) { __tty = __tty__; }
